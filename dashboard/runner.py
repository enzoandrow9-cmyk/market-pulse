...
class QuantLabRunner:
    ...
    def parse_command(self, command: str) -> dict[str, Any]:
        ...
        if action == "research":
            if len(tokens) < 4:
                raise ValueError("Usage: research SYMBOL[,SYMBOL] START_DATE END_DATE [interval=...]")
            extras = {}
            for token in tokens[4:]:
                if "=" in token:
                    key, value = token.split("=", 1)
                    extras[key] = _coerce_value(value)
            return {"action": action, "symbols": tokens[1].split(","), "start_date": tokens[2], "end_date": tokens[3], "extras": extras}
        if action == "regime":
            if len(tokens) < 4:
                raise ValueError("Usage: regime SYMBOL START_DATE END_DATE [interval=...]")
            extras = {}
            for token in tokens[4:]:
                if "=" in token:
                    key, value = token.split("=", 1)
                    extras[key] = _coerce_value(value)
            return {"action": action, "symbol": tokens[1], "start_date": tokens[2], "end_date": tokens[3], "extras": extras}
        if action == "optimize":
            if len(tokens) < 5:
                raise ValueError("Usage: optimize SYMBOL STRATEGY START_DATE END_DATE [method=grid|random|bayesian|genetic] [mode=...] [capital=...] [interval=...]")
            extras = {}
            for token in tokens[5:]:
                if "=" in token:
                    key, value = token.split("=", 1)
    ...
    def run_optimization(self, symbol: str, strategy: str, start_date: str, end_date: str, extras: dict[str, Any], raw_command: str) -> dict[str, Any]:
        ...
        selected_mode = str(extras.get("mode", "vectorized"))
        selected_capital = float(extras.get("capital", 250_000))
        selected_interval = str(extras.get("interval", "1d"))
        ...
        def objective(params: dict[str, Any]) -> float:
            result = self.run_backtest(
                ...
                start_date,
                end_date,
                {"mode": selected_mode, "capital": selected_capital, "interval": selected_interval, **params},
                raw_command,
            )
